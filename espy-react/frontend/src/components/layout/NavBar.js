...
const NavBar = () => {
  ...
  return (
    <nav
      style={{ borderBottom: "3px solid #ff2a12" }}
      className="navbar navbar-expand-lg navbar-light"
    >
      <a
        href="/"
        className="navbar-brand fw-bold px-4 text-muted"
        style={{ fontSize: "2em" }}
      >
        Espy
      </a>
      <button
        className="btn d-md-none d-sm-block me-auto"
        onClick={() =>
          $(".nav-wrapper").toggle(
            function () {
              $(".nav-wrapper").addClass("sid-active");
            },
            function () {
              $(".nav-wrapper").removeClass("sid-active");
            }
          )
        }
      >
